...
/**
 * This frame has two buttons to fill a comb box from a separate thread.The
 * "Good" button uses the eveunt queue,the "Bad" button modifies the comb box
 * directly.
 */
 ...
 * and both the worker thread and the event dispathch thread accsee the combo box.
 */
class BadWorkerRunnable implements  Runnable{
    private JComboBox<Integer> combox;
    ...
    public void run(){
        try{
            while(true){
                ...
                if(i%2==0){
                    combox.insertItemAt(i,0);
                }
                else if(combox.getItemCount()>0){
                    combox.removeItemAt(i%combox.getItemCount());
                }
                Thread.sleep(1);
            }
