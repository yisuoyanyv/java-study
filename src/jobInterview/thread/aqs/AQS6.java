 ...
 * 9.实现释放锁的方法release
 * a.尝试释放锁 tryRelease
 * b.如果释放成功则唤醒下一个线程unparkSuccessor()
 */
...
public class AQS6 {
    ...
    private void unparkSuccessor(Node head) {
        ...
        //如果下一个元素是null，或者下一个元素的状态不是wait，则从列表的最后一个元素开始往前找
        //找到最后一个状态不是Canceled的作为next
        if (next == null || next.threadStatus != Node.WAIT) {
            for (Node t = tail; t != null && t != h; t = t.prev) {
                if (t.threadStatus != Node.CANNELED) {
                    next = t;
                }
            }
        }
        ...
        //唤醒下一个元素
        if (next != null) {
            System.out.println(Thread.currentThread().getName() + "唤醒了线程：" + next.thread.getName());
            //唤醒线程
    ...
    private boolean compareAndSetState(int i, int arg) {
        return unsafe.compareAndSwapInt(this, statusOffset, i, arg);
    ...
    static {
        try {
            ...
            //获取status属性的偏移量
            statusOffset = unsafe.objectFieldOffset(AQS6.class.getDeclaredField("status"));
            //获取tail属性的偏移量
            tailOffset = unsafe.objectFieldOffset(AQS6.class.getDeclaredField("tail"));
            //获取head属性的偏移量
            headOffset = unsafe.objectFieldOffset(AQS6.class.getDeclaredField("head"));
